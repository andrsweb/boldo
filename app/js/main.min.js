(function (factory) {
	...
	menuButton.addEventListener('click', () => {
	  if (!menuBurger.classList.contains('hide')) menuBurger.classList.add('hide');else menuBurger.classList.remove('hide');
	  if (!menuButton.classList.contains('hide')) menuButton.classList.add('hide');else menuButton.classList.remove('hide');
	});
	/**
	...
	*/
	...
	const isInScope = (elementSelector, st, offset = -1300) => {
	  const element = document.querySelector(elementSelector);
	  if (!element) return;
	  if (st >= element.offsetTop - window.innerHeight + offset && st <= element.offsetTop + element.clientHeight - offset) return true;
	  return false;
	};
	...
	const questionsToggle = () => {
	  ...
	  questions.forEach(question => {
	    // forEach - cycle for each element in ('.dropdown-question')
	    question.addEventListener('click', () => {
	      if (!question.classList.contains('hide')) question.classList.add('hide');else question.classList.remove('hide');
	    });
	  });
	};
